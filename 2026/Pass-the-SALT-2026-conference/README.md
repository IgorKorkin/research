# Pass the SALT 2026 | DesktopRanger Blocks Keystroke Spying: Hardening Windows Desktop Isolation

<!-- markdownlint-disable MD033 -->
<p align="center">
  <img src="logo.png" alt="Pass the SALT 2026 logo" width="400">
  <img src="hello.jpg" alt="Igor Korkin at Pass the SALT 2026" width="400">
  <br>
  <img src="promotion.png" alt="DesktopRanger at Pass the SALT 2026" width="400">
</p>
<!-- markdownlint-enable MD033 -->

## Talk

* **Title:** DesktopRanger Blocks Keystroke Spying: Hardening Windows Desktop Isolation
* **Speaker:** Igor Korkin
* **Event:** Pass the SALT 2026
* **Type:** Conference presentation
* **Date:** July 2, 2026
* **Venue:** Université Catholique de Lille
* **Location:** Lille, France
* **Language:** English
* [Watch the recorded talk](https://passthesalt.ubicast.tv/videos/2026-desktopranger-blocks-keystroke-spying-hardening-windows-desktop-isolation/)
* [DesktopRanger on GitHub](https://github.com/IgorKorkin/DesktopRanger)
* [Official participation letter](./letter_igor_korkin.pdf)

## Abstract

The talk demonstrates how privileged user-mode keyloggers can attack isolated Windows desktops and presents DesktopRanger, an open-source prototype that strengthens access control for Window Station and Desktop objects to protect sensitive keyboard input.

## Official Posts

* [LinkedIn](https://www.linkedin.com/posts/pass-the-salt-conference_pts2026-security-by-design-session-activity-7472606299772497920-Sx0n)
* [Infosec.Exchange](https://infosec.exchange/@passthesaltcon/116759478471206454)
* [X / Twitter](https://x.com/passthesaltcon/status/2066842363641618561)

## How to Cite

### ГОСТ

<!-- markdownlint-disable MD033 -->
<table>
<tr>
<td>
Коркин И. Ю. DesktopRanger Blocks Keystroke Spying: Hardening Windows Desktop Isolation : доклад на конференции Pass the SALT 2026, Лилль, Франция, 2 июля 2026 г.
</td>
</tr>
</table>
<!-- markdownlint-enable MD033 -->

### APA 7

<!-- markdownlint-disable MD033 -->
<table>
<tr>
<td>
Korkin, I. (2026, July 2). DesktopRanger blocks keystroke spying: Hardening Windows Desktop isolation [Conference presentation]. Pass the SALT 2026, Lille, France.
</td>
</tr>
</table>
<!-- markdownlint-enable MD033 -->

<!-- markdownlint-disable MD033 -->
<p align="center">
  <a href="letter_igor_korkin.pdf">
    <img src="letter_igor_korkin.png" alt="Official participation letter for Pass the SALT 2026" width="700">
  </a>
</p>
<!-- markdownlint-enable MD033 -->